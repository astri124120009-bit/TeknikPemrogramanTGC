...
# ===============================
# PENGATURAN VISUALISASI (CHALLENGE)
# ===============================
st.sidebar.markdown("---")
...
with tab2:
    ...
    sc = ax_sc.scatter(df_input["x"], df_input["y"], c=df_input["t_obs"], cmap=cmap_option)
    fig_sc.colorbar(sc, ax=ax_sc, label="nT")
    st.pyplot(fig_sc)
...
method = st.selectbox("Metode", ["2D Moving Average", "Trend Surface Analysis"])
...
with col1:
    ...
    c2 = ax2.contourf(X, Y, Regional, levels=20, cmap=cmap_option, vmin=vmin, vmax=vmax)
...
if st.button("💾 Simpan Peta Residual"):
    fig3.savefig("peta_residual.png", dpi=300, bbox_inches="tight")
